...
/// Dates written in digits. The day/month order follows the active locale, so
/// this reads the same in every language.
func numericDateMatches(
    in text: String,
    range: NSRange,
    calendar: Calendar,
...
func clockTime(_ value: String, meridiem: EventTitleMeridiem?) -> EventTitleTime? {
...
func monthNumber(matching input: String, calendar: Calendar, localeIdentifier: String) -> Int? {
...
func localizedWeekdayCandidates(calendar: Calendar, localeIdentifier: String) -> [EventTitleWeekdayCandidate] {
    let formatter = DateFormatter(calendar: localizedCalendar(calendar, localeIdentifier))

    var result: [EventTitleWeekdayCandidate] = []
    for (index, name) in formatter.weekdaySymbols.enumerated() {
        result.append(.init(name: normalizedParserWord(name), weekday: index + 1, allowsFuzzyMatching: true))
    }
    for (index, name) in formatter.shortWeekdaySymbols.enumerated() {
        result.append(.init(name: normalizedParserWord(name), weekday: index + 1, allowsFuzzyMatching: false))
    }
    return result
}

func weekdayNumber(matching input: String, in candidates: [EventTitleWeekdayCandidate]) -> Int? {
...
func normalizedParserWord(_ value: String) -> String {
    value
...
func normalizedTimeValue(_ value: String) -> String {
...
private func normalizedDateWord(_ value: String) -> String {
    normalizedParserWord(value).trimmingCharacters(in: CharacterSet.punctuationCharacters)
}
...
func overlaps(_ range: NSRange, _ excludedRange: NSRange?) -> Bool {
    guard let excludedRange else { return false }
    return NSIntersectionRange(range, excludedRange).length > 0
}

func validRange(_ range: NSRange) -> NSRange? {
