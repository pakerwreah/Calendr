...
private func dateMatches(
    ...
    for candidate in dateExpressions {
        ...
        where !EnglishEventTitleParser.isExcluded(
            match.range, by: excludedRanges + results.map(\.range))
        {
    ...
    where !EnglishEventTitleParser.isExcluded(match.range, by: excludedRanges + results.map(\.range)) {
        guard
